...
export function VotingGuide({
  ...
  const chooseVotingMethod = useCallback(
  (
    nextMethod: VotingMethod,
    trigger: HTMLButtonElement,
  ) => {
    /*
     * Computers bypass calendar reminders and navigate directly
     * to the selected official voting method.
     *
     * Phones and tablets continue through the reminder flow.
     */
    if (!isMobileOrTabletDevice()) {
      navigateToOfficialVoting(nextMethod);
      return;
    }
  ...
  return (
    <div className="page-shell min-h-screen">
      <a
        href="#main-content"
        className="fixed left-3 top-3 z-[60] -translate-y-24 rounded-full bg-white px-4 py-3 font-bold text-[#5f3d88] shadow-lg focus:translate-y-0"
      >
        {language === "es"
          ? "Ir al contenido"
          : "Skip to content"}
      ...
      <header className="sticky top-0 z-40 border-b border-[#e6dce9]/80 bg-[#fffaf5]/90 backdrop-blur-xl">
        <div className="section-wrap flex min-h-18 items-center justify-between gap-3 py-3">
          <a
            href="#main-content"
            className="flex min-h-11 items-center gap-2 font-black tracking-tight text-[#5f3d88]"
          >
            <Heart
              aria-hidden="true"
              className="fill-[#d45d79] text-[#d45d79]"
              size={20}
            />
          ...
          <LanguageToggle
            language={language}
            englishLabel={
              copy.header.languageEnglish
            }
            spanishLabel={
              copy.header.languageSpanish
            }
      ...
      <main id="main-content">
        ...
        <EstimatedNextVote
          language={language}
          copy={copy.estimatedNextVote}
          reminderState={reminderState}
        />

        <section
          className="section-wrap py-12"
          aria-labelledby="choose-method-heading"
        >
          <h2
            id="choose-method-heading"
            className="section-title text-center"
          >
            {copy.chooseOption.heading}
          </h2>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-2">
            <VotingMethodCard
              method="facebook"
              title={
                copy.chooseOption.facebookTitle
              }
              steps={
                copy.chooseOption.facebookSteps
              }
              note={
                copy.chooseOption.facebookSafety
              }
              buttonLabel={
                copy.hero.voteFacebook
              }
              onVote={chooseVotingMethod}
            />
        ...
        <StepGuide
          heading={
            copy.profileWalkthrough.heading
          }
          steps={copy.profileWalkthrough.steps}
          stepLabel={copy.common.step}
          buttonLabel={
            copy.profileWalkthrough.openProfile
          }
        />

        <section
          className="section-wrap py-6"
          aria-labelledby="daily-vote-heading"
        >
          <div className="flex items-start gap-4 rounded-[1.5rem] bg-[#5f3d88] p-6 text-white shadow-xl sm:p-8">
            ...
            <div>
              <h2
                id="daily-vote-heading"
                className="text-2xl font-black"
              >
                {copy.freeVoteReminder.heading}
              </h2>

              <p className="mt-2 text-lg leading-8 text-[#f4ebfb]">
                {copy.freeVoteReminder.text}
              </p>
            </div>
          </div>
        </section>
